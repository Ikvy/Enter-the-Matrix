...
public:
    ...
    T determinant() const { //gaussian elimination like above but with no normalisation (which changes the determinant)
        ...
        int swap_count = 0;
        const float EPS = 1e-6f; // Small threshold to avoid numerical instability due to float (f32 imposed in the subject)
    ...
    Matrix<T> inverse() const {  
        ...
        const float EPS = 1e-6f; //always float handling
